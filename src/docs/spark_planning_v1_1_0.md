...
| 단계 | 내용 |
|---|---|
| MVP (지금) | 로그인 → 홈 → 에디터 (이어쓰기 + 피드백) → 아카이브 |
...
③ 스캔 애니메이션 (API 호출 중)
   ...
   - 스캔 라인: 검정 반투명 위→아래
   - 헤더 점 애니메이션 + "글을 읽고 있어요"
④ 피드백 결과 표시
   - 상단: 구조 배너 1줄 (#f4f4f5 + 블랙 왼쪽 선)
   - 원문에 하이라이트 직접 표시
...
│   ├── editor/
│   │   ├── page.tsx                ✅
│   │   └── EditorClient.tsx        ✅ (피드백 모드 업데이트 필요)
│   ├── archive/
│   │   ├── page.tsx                ✅
│   │   └── ArchiveClient.tsx       ✅
│   ├── actions/
...
| 파일 | 내용 |
|---|---|
| `src/app/api/feedback/route.ts` | 응답 구조 변경 (flow + highlights[이유+예시] + next) |
| `src/app/editor/EditorClient.tsx` | 피드백 모드 전환 UI (헤더 블랙, 스캔, 하이라이트, 수정창) |
